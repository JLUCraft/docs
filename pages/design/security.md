...
**信任根运行时注入，信任链密码学验证，威胁靠分层机制兜底**。

## 信任模型概览

```mermaid
flowchart TB
  subgraph Injection["信任根注入"]
    E["环境变量 JLUCRAFT_GENESIS_PUBKEYS<br/>或 CLI --genesis-pubkeys"]
  end
...
所有操作都可追溯到多签签名或有效 VC 的身份。内置信任根列表为空；生产密钥由运维在启动时注入。
...
- **信任根**：核心管理成员公钥通过环境变量 `JLUCRAFT_GENESIS_PUBKEYS` 或 CLI `--genesis-pubkeys` 注入，不在源码中携带。编译产物中不含默认信任根（内置列表为空）。修改信任根需要协调所有节点重启，无法单点变更——物理边界。
...
- **物理机边界**：federated-server 节点保持零本地配置——所有治理参数、准入规则、实例配置均来自共识层，节点启动时从 Raft 快照加载，运行时不读取本地可写配置文件。物理机的控制者最多能让节点离线，无法通过修改本地文件来篡改链上数据或绕过多签约束。
...
::: warning 信任根注入
信任根不硬编码在二进制中。生产部署时，密钥通过以下方式注入：

- **环境变量** `JLUCRAFT_GENESIS_PUBKEYS="key1,key2,..."`: 容器 / systemd 部署
- **CLI 标志** `--genesis-pubkeys key1,key2,...`: 手动部署

在 `debug` 构建中，空的信任根仅产生警告（方便本地开发）；在 `release` 构建中，空信任根会导致启动失败。占位符密钥（RFC 8032 TEST 1）在 release 构建中一律被拒绝，除非显式传递 `--dev-insecure-local-config`。

**密钥轮换流程**：
1. 核心成员生成新密钥对
2. 所有节点在 `JLUCRAFT_GENESIS_PUBKEYS` 中包含新旧两套密钥
3. 所有节点按共识节奏重启（建议逐个灰度）
4. 旧密钥下的多签提案仍然有效；新提案使用新密钥
5. 过渡期结束后从信任根中移除旧密钥

**部署前检查清单**：
1. 为每位核心成员生成 Ed25519 密钥对
2. Base64 编码每个**公钥**（32 字节）
3. 在 release 构建中验证占位符被拒绝：
   ```bash
   JLUCRAFT_GENESIS_PUBKEYS="11qYAYKxCrfVS/7TyWQHOg7hcvPapjIcrwIaaPcHURo=" \
     ./federated-server  # 应该报错并退出非零码
   ```
4. 用真实密钥验证二进制正常启动
