...
flowchart TB
  ...
  M["Member · 管理员验证<br/>权限：BASIC<br/>查看服务器 · 创建房间<br/>加入服务 · 参与比赛<br/>普通成员治理参与"]
  G["Guest · MUA Yggdrasil<br/>权限：GUEST<br/>加入公开/互通实例<br/>无治理权限 · 无积分"]

  P -->|任命/罢免| A
  A -->|验证| M
  M -.->|无 VC / 仅 Yggdrasil| G
```
...
持有 `MemberCredential` 的玩家通过 FollyLauncher 参与普通成员治理。此类治理只覆盖成员范围内的投票、反馈、申请和自身相关事项，不授予管理员多签、节点准入、系统配置或实例强制运维权限。
