...
federated-server 的 REST/JSON HTTP API 监听地址由 `config.toml` 中的 `[observability].listen` 配置项决定，**默认值为 `127.0.0.1:9090`**。
...
| JoinInstance | 加入实例，返回本地代理端口(MC 连接 localhost:port)。实例以 `name` 展示，以 `id` 寻址 |
| CreateQuickRoom | 快速创建房间，返回实例 ID（`name` 可后续修改） |
