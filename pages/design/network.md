...
**注意：** 跨端内部通讯统一使用 protobuf over libp2p，不使用 HTTP 控制面、gRPC 或 libp2p PubSub 承载业务语义。HTTP 只允许保留一个只读 `/status` 状态接口，供监视网页显示节点状态；它不能承载控制、管理、查询、订阅或授权。FollyLauncher 前端仅通过 Tauri IPC 与 Rust core 通信，Rust core 再通过 protobuf over libp2p 与服务器节点协同。

**设计决策：**

- **QUIC**：支持连接迁移，NAT 重绑定时不断连；多径支持适合"校园网+内网穿透"场景。
- **Noise 握手**：轻量级，比 TLS 握手更快，适合延迟敏感场景。
- **yamux 多路复用**：单连接承载多条逻辑流（控制请求、事件流、游戏数据、心跳分通道走）。

### 内部通讯协议边界

所有跨端内部通讯必须满足以下约束：

- 控制请求 / 响应：`ControlRequest` / `ControlResponse` 以 protobuf 帧封装在 libp2p stream 中。
- 事件订阅：`SubscribeEvents` 返回 `EventEnvelope` 流；`topic` 仅作过滤标签，不对应 PubSub 频道。
- 游戏数据面：MC 协议仍作为字节流透传在 libp2p stream 中，不转换为 HTTP 或 gRPC。
- 推送通知：UnifiedPush 只用于系统通知展示和拉起应用，不能作为状态、授权或事件的可靠来源。
- HTTP 例外：只读 `GET /status` 仅供监视网页展示摘要状态，除此之外不得暴露 HTTP 端点。
...
未来可扩展的方向：WireGuard overlay、其他高校 MUA 联合大厅等。
