...
| `member` | 完整功能（游戏 + 普通成员治理 + 积分） |
| `expired` | 仅可访问历史会话，需续签恢复 |
| `revoked` | 阻止所有连接，本地清空 VC |

> **治理功能归属**：治理操作（提案投票、多签审批、成员管理、VC 签发、审计日志）通过 `union-manager` 管理控制台完成，不在启动器（FollyLauncher）内提供。启动器是玩家端程序，专注身份认证、游戏启动和网络连接。持有 `member` VC 可解锁成员专属实例的加入权限，但不会在启动器内开放治理 UI。
...
平台 VC 绑定启动器本地 PeerID 后，FollyLauncher 即成为普通成员治理客户端。普通成员可以查看链上治理状态、参与成员范围内的投票或反馈、提交与自身身份、房间和联赛相关的申请；涉及管理员任免、节点准入、配置变更和实例强制运维的高权限治理仍必须使用管理终端完成。
