...
FollyLauncher 是玩家接入本系统的入口，也是普通成员参与链上治理的客户端。它在本地完成密钥派生、节点发现、协议代理、资源同步，让 MC 原版客户端"无修改"地接入对等网络。
...
启动器是一台**完整的对等节点**：持有自己的密钥，参与 DHT 寻址，与服务器节点进行 QUIC 直连或中继通信。持有平台 VC 的用户可在启动器内以普通成员身份查看链上治理状态、参与成员投票或提交申请；高权限管理动作仍由 union-manager 负责。所有功能模块在本地协作完成，不依赖任何中心化 API。
...
| [主界面](./ui) | 服务器 / 房间 / 联赛 / 治理 / 我的 页面、通知系统、资源管理 |
