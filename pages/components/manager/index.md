...
管理终端是社长和管理员的移动 / 桌面工具,承载平台的全部管理职能:身份签发、节点运维、提案多签、联赛组织、审计追溯。管理终端是一台**带 TEE 保护的对等节点**,私钥不出设备,所有操作走多签共识。
...
管理终端是平台中**权限最高**的角色,管理员单签或多签可以决定节点准入、资金 / 积分发放、实例存活。这种权力不能依赖普通的密码学密钥保护——一台被植入木马的手机就能让攻击者拿到主权。
