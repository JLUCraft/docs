...
服务器节点的"对外面孔"是 libp2p Host：它将入站流量落地为 MC 实例的本地 TCP 连接，将跨节点协作流量（共识、中继、存储）分发到对应的内部模块。
...
超过阈值的连接被拒绝，并将对方 PeerID 临时拉黑（默认 5 分钟），拉黑事件写入本地审计日志。
...
服务器节点将所有持久化操作收敛到 `StorageClient` 模块——它封装 S3 API、本地磁盘缓存、写入并发控制三件事。
...
实例运行期间 `StorageClient` 持续将 WAL（玩家操作流）异步写入 S3 `wal/<instance>/<ts>.log`。即便节点突然断电导致世界文件未同步，新宿主启动后也可以从 WAL 重放最后几秒的玩家操作，将丢失数据控制在秒级而非分钟级。
