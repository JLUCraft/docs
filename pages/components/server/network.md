...
federated-server 仅保留一个只读 HTTP 接口：

| 方法 | 路径 | 消费方 | 说明 |
| --- | --- | --- | --- |
| GET | `/status` | 监视网页 | 展示节点在线状态、PeerID、版本、实例数量、共识角色和最近告警摘要 |

该接口只服务于人类可读的监视页面，不是权威状态同步通道。控制、管理、授权、实例列表、事件订阅、metrics、health 和 readiness 都不得通过 HTTP 暴露；需要权威数据的客户端必须使用 protobuf over libp2p。
...
| 子协议 | substream | 数据 |
| --- | --- | --- |
| 心跳 / AppendEntries | `/raft/v1/append` | leader → followers，生产默认 1000 ms，低延迟 LAN 可进一步下调 |
| 投票 | `/raft/v1/vote` | candidate → 全员 |
| 快照传输 | `/raft/v1/snapshot` | leader → 落后者，大对象（>16 MiB）时切到 S3 直链 |
...
每 10000 条日志生成一次快照，快照本身上传到 S3 `snapshots/raft/{group_id}/{index}-{term}.bin`，Raft 日志条目仅保留指针。新节点先拉取快照再追加增量日志，避免初次同步时耗尽 libp2p 带宽。
...
实例运行期间 `StorageClient` 持续将 WAL（玩家操作流）异步写入 S3 `wal/{instance_id}/lifecycle-{timestamp}-{uuid}.json` 与 `wal/{instance_id}/migration-{timestamp}-{uuid}.json`。即使节点突然断电导致世界文件未同步，新宿主启动后也可从 WAL 重放最近几秒的玩家操作，数据丢失控制在秒级以内。

## 去中心化推送通知（UnifiedPush）

federated-server 需要提醒管理终端处理 AuthChallenge、告警或治理事件时，通过 **UnifiedPush** 发送系统级推送通知。通知 payload 可以携带用于展示的业务摘要，例如标题、正文、严重级别、对象名称和时间；这些字段只服务于系统通知 UI，不作为权威状态。管理终端被拉起后，必须通过 protobuf over libp2p 拉取完整 challenge、告警、提案或赛事状态。

### 推送注册

union-manager 启动后通过 protobuf over libp2p 向已知服务器节点注册推送端点：
...
sequenceDiagram
    ...
    Mgr->>Srv: RegisterPushEndpoint{peer_id, endpoint, distributor_type}<br/>(protobuf over libp2p)
...
`push_endpoint` 必须是 UnifiedPush distributor 的 URL。服务器仅支持 `unified-push` distributor_type，统一使用 distributor 要求的投递方式发送通知。

### 推送分发

当需要提醒管理员时（AuthChallenge、实例崩溃、新提案等），federated-server 的 `PushDispatcher` 按以下流程分发：

```mermaid
flowchart TB
    E[触发通知事件]
    I[写入权威状态<br/>共识日志 / 本地审计]
    T{查询 push_registry}
    UP[UnifiedPush<br/>投递标题 / 正文 / 摘要]
    A[记录投递尝试]
...
推送仅使用 UnifiedPush 通道，不回退到 PubSub 广播、内部事件主题或其他手机推送通道。推送失败只影响设备是否及时看到系统通知或被拉起，不影响权威状态；管理员打开应用后仍可通过 protobuf over libp2p 同步待处理事项。

### 去中心化的安全性

- 推送 payload 可以包含展示用标题、正文和业务摘要，但不得包含可直接执行的授权材料、私密凭据或唯一权威结论
- 管理终端收到通知后通过 protobuf over libp2p 回连服务器，拉取完整内容并校验权限链；本地只把推送内容当作通知文案
...
- 通知不是可靠数据源，不参与状态收敛；状态收敛只依赖共识日志和 protobuf over libp2p 拉取
