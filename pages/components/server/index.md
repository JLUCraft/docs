...
服务器节点是平台的**计算底座**:它运行在所有社团提供的物理机或云主机上,承载 Docker 化的 MC 实例、参与共识组、采集预言机数据、为客户端提供网络落地点。
...
**可观测性**
所有运行时事件以**结构化日志 + Prometheus 兼容指标**输出。社团运维可以接到自己已有的 Grafana / Loki 栈,平台不强加观测后端。
